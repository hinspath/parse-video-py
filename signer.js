...
// 1. 补全浏览器环境 (防止报错)
var window = this;
var document = { referrer: 'https://www.douyin.com/' };
var navigator = { userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36' };

// 2. 导出签名函数
function get_sign(url, ua) {
    // 提取 query 参数字符串
    var query = url.indexOf("?") !== -1 ? url.split("?")[1] : url;
    
    // 调用源码里的生成函数 (函数名通常是 generate_a_bogus)
    if (typeof generate_a_bogus === 'function') {
        return generate_a_bogus(query, ua);
    } 
    // 兼容部分旧版本文件名
    else if (typeof getXBogus === 'function') {
        return getXBogus(query, ua);
    }
